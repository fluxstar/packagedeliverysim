...
Auctioneer::Auctioneer(SimulationModel* model, int maxWait)
    : model(model), maxAllottedWait(maxWait) {}
...
void Auctioneer::addDrone(AuctionDrone* drone) { drones.push_back(drone); }

void Auctioneer::auctionAssignment(std::vector<AuctionDrone*> drones,
                                   std::vector<Package*>& packages,
                                   std::vector<int> prices) {
  std::vector<AuctionDrone*> currentAssignments;
  for (int i = 0; i < packages.size(); i++) {
    currentAssignments.push_back(
        nullptr);  // Initialize all packages to be unassigned
  }
  ...
  for (int i = 0; i < MAX_ROUNDS; i++) {
    for (int d = 0; d < drones.size(); d++) {
      ...
      for (int j = 0; j < prices.size(); j++) {
        ...
        if (value >
            maxValue) {  // This function is to find the most valuable package
          maxValue = value;
          maxPackageIndex = j;
        }
      }
      ...
      // Calculate the new price for the package (equal to the current price of
...
void Auctioneer::update(double dt) {
  ...
  // If there is a drone to deliver then remove the counter and the package from
  // the queue Use the already computed data to send all packages that have
  // already expired
  ...
  std::vector<int> prices;
  if (!this->packages.empty() && this->waitTimes.front() <= 0 &&
      drones.size() > 0) {
